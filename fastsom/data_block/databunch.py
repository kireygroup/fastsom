...
This section is obsolete and is kept for retro-compatibility reasons only,
    ...
    def process(self, ds: TabularList):
        ...
        for i, proc in enumerate(self.procs):
            if isinstance(proc, TabularProc):
                ...
                print(
                    proc.__class__.__name__, len(proc.cat_names), len(proc.cont_names)
                )
        ...
        last_tobecont_proc = find(
            self.procs, lambda p: isinstance(p, ToBeContinuousProc), last=True
        )
        if last_tobecont_proc is not None:
            ds.cont_names = (
                last_tobecont_proc.cont_names
            )  # + last_tobecont_proc.transformed_cat_names
            ds.cat_names = []
