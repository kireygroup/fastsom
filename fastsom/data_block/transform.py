...
class Vectorize(ToBeContinuousProc):
    ...
    def apply_train(self, df):
        ...
        self._category_values = {
            col: list(set(df[col].unique().astype(str)) - set(["nan"]))
    ...
    def apply_test(self, df: pd.DataFrame):
        ...
        vector_sizes = [
            range(self._embedding_model.vector_size) for _ in range(len(self.cat_names))
        ]
        transformed_cat_names = (
            np.array(
                [
                    [f"{col}_feature{i+1}" for i in r]
                    for col, r in zip(self.cat_names, vector_sizes)
                ]
            )
            .flatten()
            .tolist()
    ...
    def _check_module(self) -> None:
        ...
        try:
            ...
            raise ImportError(
                f"You need to install gensim to use the {self.__class__.name__} \
    ...
    def _init_embedding_model(self, df: pd.DataFrame):
        ...
        if self._embedding_model is None:
            ...
            self._embedding_model = FastText(
                size=vector_size, batch_words=1_000, min_count=1, sample=0, workers=10
            )  # , min_n=2, max_n=3)
            ...
            self._embedding_model.train(
                sentences=self._get_sentences(df), total_examples=df.shape[0], epochs=8
            )

    def _sentences_to_vecs(
    ...
    def _vecs_to_sentences(self, vectors: np.ndarray) -> List[List[str]]:
        ...
        for values, col in zip(
            slices(vectors.transpose(), self.vector_size), self.original_cat_names
        ):
            row = []
            for vec in np.array(values).transpose():
                words = self._embedding_model.wv.similar_by_vector(vec, topn=topn)
                match = find(
                    words, lambda w: w[0].split("__")[-1] in self._category_values[col]
                )
                if match is None:
    ...
    def _get_sentences(self, df: pd.DataFrame) -> Generator[List[str], None, None]:
        ...
        for i in range(df.shape[0]):
            yield list(
                map(
                    lambda o: f"{o[1]}__{o[0]}",
                    zip(df[self.cat_names].values[i], self.cat_names),
                )
            )
